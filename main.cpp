...
std::vector<flocking::Boid> create_flock(flocking::running_parameters const& parameters){
...
// Function to create parameters for the simulation (custom or default)
flocking::running_parameters create_parameters(){
  ...
  std::cin>>input;
  if(input=="d"){
    return flocking::running_parameters{};
  }
  ...
  return flocking::running_parameters{boid_number,separation,alignement,cohesion};
}

// Struct to hold calculated data about the flock
...
data build_data(std::vector<flocking::Boid> const& flock){
  double mean_distance{post::calculate_mean_distance(flock)};
  double sigma_mean_distance{post::calculate_standard_deviation_distance(flock,mean_distance)};
  double mean_velocity{post::calculate_mean_velocity(flock)};
  double sigma_mean_velocity{post::calculate_standard_deviation_velocity(flock,mean_velocity)};
...
std::string print_data_to_string(data const& to_be_printed,flocking::running_parameters const& parameters) {
...
void render_boids(std::vector<flocking::Boid>& flock, flocking::running_parameters const& parameters,sf::RenderWindow& simulation_window){
  ...
  // Every flocking::Boid inside flock gets assigned a boids sf::CircleShape that are
  // drawn at the same position of the flocking::Boid
...
void render_data(std::vector<flocking::Boid> const& flock,sf::RenderWindow& data_window,sf::Font const& font,flocking::running_parameters const& parameters){
  sf::Text text;
  text.setFont(font);
...
void run_simulation(std::vector<flocking::Boid>& flock, flocking::running_parameters const& parameters) {
  ...
  while (simulation_window.isOpen() || data_window.isOpen()) {
    ...
    flocking::evolve_flock(flock, frame_time_double, parameters);
    render_boids(flock,parameters,simulation_window);
...
int main(){
  ...
  flocking::running_parameters parameters=create_parameters();
  std::vector<flocking::Boid> flock=create_flock(parameters);
  std::cout << "\nThe simulation will start now\n";
  std::cout << "\nBoids Number:"<<parameters.boids_number<<" s:"<<parameters.c<<" a:"<<parameters.a<<" c:"<<parameters.c<<" \n";
  try{
    run_simulation(flock, parameters);
  }
