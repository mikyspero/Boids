...
namespace view {
// Function to create parameters for the simulation
dynamics::running_parameters create_parameters() {

  std::string input;
  std::cout << " Good day, this is a boids program would you like to use the "
               "default settings or custom ones?\n";
  std::cout << " to use the default enter a lowercase d, "
  ...
  int boid_number;
  std::cout << "To manually create a flock of boids please first insert their "
               "number, it must be an integer equal or greater than 3, its "
               "default value is 120, the maximum number is 150\n";
  std::cin >> boid_number;
  while (std::cin.fail() || boid_number < 3 || boid_number>150) {
    std::cin.clear();
    std::cin.ignore();
    std::cout << "Please insert a valid boid number, it must be an integer "
                 "between 3 and 150\n";
    std::cin >> boid_number;
  }

  double separation;
  std::cout << "Now please insert the value of the separation parameter, it's "
               "a positive real number, its default value is 0.5\n";
  std::cin >> separation;
  while (std::cin.fail() || separation < 0 || separation > 1) {
    std::cin.clear();
    std::cin.ignore();
    std::cout << "Please insert a valid value, it must be a positive real "
                 "number between 0 and 1, its default value is 0.5\n";
    ;
    std::cin >> separation;
  }

  double alignement;
  std::cout << "Now please insert the value of the alignement parameter, it's "
               "a positive real number, its default value is 0.6\n";
  std::cin >> alignement;
  while (std::cin.fail() || alignement < 0 || alignement > 1) {
    std::cin.clear();
    std::cin.ignore();
    std::cout << "Please insert a valid value, it must be a positive real "
                 "number between 0 and 1, its default value is 0.6\n";
    ;
    std::cin >> alignement;
  }

  double cohesion;
  std::cout << "Now please insert the value of the cohesion parameter, it's a "
               "positive real number, its default value is 0.02\n";
  std::cin >> cohesion;
  while (std::cin.fail() || cohesion < 0 || cohesion > 1) {
    std::cin.clear();
    std::cin.ignore();
    std::cout << "Please insert a valid value, it must be a positive real "
                 "number between 0 and 1, its default value is 0.02\n";
    ;
    std::cin >> cohesion;
  }

  return dynamics::running_parameters{boid_number, separation, alignement,
                                      cohesion};
}
...
void render_boids(std::vector<dynamics::Boid> const &flock,
  ...
  // calculation of the scales necessary to render the boids on screen
  auto const x_scale = (.75 * sf::VideoMode::getDesktopMode().width) /
                       (parameters.right_bound - parameters.left_bound);
  auto const y_scale = (.75 * sf::VideoMode::getDesktopMode().height) /
                       (parameters.upper_bound - parameters.bottom_bound);

  sf::CircleShape boid_shape{2.0f, 5};
  boid_shape.setFillColor(sf::Color(sf::Color::White));
  // The simulation_window with the objects of the previous frame is cleared and
  // filled with our
  simulation_window.clear(sf::Color(sf::Color::Black));
  // Every dynamics::Boid inside flock gets assigned a boids sf::CircleShape
  // that are drawn at an equivalent position as their BOid
...
data build_data(std::vector<dynamics::Boid> const &flock) {
  ...
  double sigma_mean_distance{
      view::calculate_standard_deviation_distance(flock, mean_distance)};
  double mean_velocity{view::calculate_mean_velocity(flock)};
  double sigma_mean_velocity{
      view::calculate_standard_deviation_velocity(flock, mean_velocity)};
  data built{mean_distance, sigma_mean_distance, mean_velocity,
             sigma_mean_velocity};
  return built;
}
...
// function to display the data window
void render_data(std::vector<dynamics::Boid> const &flock,
                 sf::RenderWindow &data_window, sf::Font const &font,
                 dynamics::running_parameters const &parameters) {
  sf::Text text;
  text.setFont(font);
...
void run_simulation(std::vector<dynamics::Boid> &flock,
  ...
  sf::RenderWindow simulation_window(
      sf::VideoMode(display_width, display_height), "Boids Simulation");
  sf::RenderWindow data_window(
      sf::VideoMode(.33 * display_width, 0.22 * display_height),
      "Data Display");
  ...
  // Starting the clocks needed to evolve the simulation and update data
  ...
  // render of the starting conditions
  render_boids(flock, parameters, simulation_window);
  // Game loop, while both windows are open the simulation is rendered
  while (simulation_window.isOpen() || data_window.isOpen()) {
    ...
    // let's measure the elapsed time in a frame and convert it in a double
    ...
    // every two seconds the data are updated for half a second
